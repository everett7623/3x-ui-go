...
# 修改后的防火墙配置，增加对常用端口（包括443）的支持，或选择性关闭防火墙
echo -e "${YELLOW}[优化] 配置防火墙规则${PLAIN}"
if [[ "${release}" == "centos" ]]; then
    ...
    # 设置默认策略
    iptables -P INPUT ACCEPT    # 修改为默认接受，更友好的配置
    ...
    iptables -A INPUT -p tcp --dport 18888 -j ACCEPT  # 默认面板端口
    ...
    echo -e "${GREEN}防火墙已配置为允许所有端口，您可以安全地使用任意端口${PLAIN}"
else
